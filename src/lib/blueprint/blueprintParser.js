...
// 解析可见性掩码（uint32 位图）→ { editor: {轨道id: 可见}, inGame: {轨道id: 可见} }
function parseVisibility(reader) {
  const editorMask = reader.readUInt32();
  const inGameMask = reader.readUInt32();
  const toLayerMap = (mask) => {
    const map = {};
  ...
  return {
    editor: toLayerMap(editorMask),
    inGame: toLayerMap(inGameMask),
  };
}
 ...
 *
 * Visibility = { editor: {轨道id: 可见}, inGame: {轨道id: 可见} }  // 由掩码解析而来
 * RGBA       = { r, g, b, a }  // 0-255
 * HSVA       = { h, s, v, a }  // 0-1
 *
